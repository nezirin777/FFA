    ...
    def simulate(self):
        ...
        for turn_idx in range(1, s.turn + 1):
            ...
            if kclt_ritu > random.randrange(100):
                s.com1 += f"<br><font color=\"{config.Config['color_red']}\" size=4><b>クリティカルヒット！！</b>「{s.chara['comment']}」</font>"
                s.dmg1 = s.dmg1 * 3
                
            if s.is_player_enemy:
                mclt_ritu = 100 - int(s.mhp / s.winner["max_hp"] * 100) if s.winner["max_hp"] > 0 else 0
                if mclt_ritu > random.randrange(100):
                    s.com2 += f"<br><font color=\"{config.Config['color_red']}\" size=4><b>クリティカルヒット！！</b>「{s.winner['comment']}」</font>"
                    s.dmg2 = s.dmg2 * 3
            else:
                mclt_ritu = 100 - int(s.mhp / s.mhp_flg * 100) if s.mhp_flg > 0 else 0
                if mclt_ritu > random.randrange(200):
                    s.com2 += f"<br><font color=\"{config.Config['color_red']}\"><b>クリティカルヒット！！</b></font>"
            ...
            if sake1 > random.randrange(300):
                s.dmg2 = 0
                s.com2 += f"<br><font size=3 color=\"{config.Config['color_red']}\"><b>{s.chara['name']}は攻撃をかわした！</b></font>"
                
            # 敵回避判定
            if sake2 > random.randrange(100):
                s.dmg1 = 0
                s.com1 += f"<br><font size=3 color=\"{config.Config['color_red']}\"><b>{s.mname}は攻撃をかわした！</b></font>"
