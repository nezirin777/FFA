        ...
        if win == 1:
            ...
            if is_genei:
                if random.randrange(3) == 0:
                    ...
                    gold_gained += otakara
                    comment += f'<br><b><font size=5 color=red>宝箱を発見！ {otakara} ゴールドを獲得しました！！</font></b><br>'
                else:
                    comment += '<br><b><font size=5 color="#aaaaaa">宝箱は空っぽだった・・・</font></b><br>'

            chara["gold"] += gold_gained
            if chara["gold"] > config.Config['max_gold']:
                chara["gold"] = config.Config['max_gold']
            
            comment += f'<font color="{config.Config['color_green']}" size=5>戦闘に勝利しました！</font><br>'
            comment += f'経験値 {exp_gained} と {gold_gained} ゴールドを獲得しました。<br>'
        elif win == 0:
            comment += f'<font color="{config.Config['color_red']}" size=5>戦闘に敗北しました・・・</font><br>'
        else:
            comment += f'<font color="{config.Config['color_yellow']}" size=5>時間切れ引き分けです。</font><br>'
