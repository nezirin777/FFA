...
{table(["境界", "実装"], [
    ...
    ["管理者", "admin.pyは管理者パスワードを確認し、POSTの改変操作にはCSRF検証を追加する。"],
    ["入力値", "ID、パスワード、名前、数値、職業、装備ID、JSONマスターなどを各処理で検証する。"],
    ["表示値", "テンプレートの自動エスケープ、HTML出力境界のescape、battle commentのhtml.escapeなどで表示用データを扱う。"],
], row_headers=True)}
<h2>排他制御と保存</h2>
{table(["対象", "ロック名・方式", "目的"], [
...
{table(["項目", "現状・対策"], [
    ...
    ["URL上の管理者パスワード", "管理画面の画面遷移ではpassをクエリ/hiddenで引き回す箇所がある。アクセスログやRefererへの露出を避ける改善余地がある。"],
    ["ファイル公開範囲", "save_data、lock、バックアップはWebから直接取得できない配置とサーバー設定を確認する。"],
