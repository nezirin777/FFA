...
def battle_logic_comparisons() -> dict[str, dict[str, str]]:
    ...
    items = {
        ...
        "チャンピオン戦の報酬・王者更新": ("時間切れ引分を王者交代なしへ分離", "不具合修正", "確認済み", "win=3の時間切れは通常EXPのみを得て、賞金・王者交代・王者連勝を発生させない。相打ち引分win=2は従来どおり新王者交代として扱う。敗北EXPの自分Lv×10上限は意図的な調整。"),
        "天下一武道会の報酬・ラウンド進行": ("時間切れ引分を賞金・進行なしへ分離", "不具合修正", "確認済み", "win=3の時間切れは通常EXPのみを得て、賞金・盗み・boss_flag進行を発生させない。相打ち引分win=2の盗み差分処理とは区別する。"),
...
def ownership_progression_comparisons() -> dict[str, dict[str, str]]:
    ...
    results = {
        ...
        "人間チャンピオン": {
            "difference": "winner.cgiの連番レコードからchampion.jsonの名前付き状態へ移行。時間切れを専用引分へ分離",
            "intent": "不具合修正",
            "status": "確認済み",
            "note": "通常の勝利・相打ち引分は挑戦者を王者へ交代し、敗北では王者の連勝・最高連勝を更新する。時間切れwin=3は引分表示として、王者交代・賞金・連勝を発生させず通常EXPのみを得る。勝利・相打ち引分EXPは相手Lv×基準値、敗北EXPだけmin(相手Lv, 自分Lv×10)へ調整済み。",
        },
        "天下一武道会の参加者・対戦履歴": {
