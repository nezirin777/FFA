...
FILE_AUDITS = (
    {
        ...
        "intent": "不具合を修正済み",
        "note": "2d1a86cでmaster_tactics_enabledを転職時の戦術リセット設定から分離し、Ver2と同じくLv60以上の他職戦術を候補にするよう修正した。POST時はロック後に最新chara/syokuで候補を再構築して再検証し、接続元も保存する。",
    },
    {
        "file": "cgi_py/tensyoku.py",
        "v2_source": "旧版_ver2/tensyoku.cgi",
        "scope": "転職条件、職業Lv退避・復帰、能力減少、戦術、接続元ホスト",
        "difference": "転職成功時の接続元ホスト保存が欠けていた。Ver2はmaster_tacで戦術リセットも制御するが、現行は候補利用と転職時リセットを別設定に分離する。カルマ下限はVer2と異なる。現行は経験済み職への再転職時に能力・マスター前提を免除する",
        "intent": "ホストは不具合を修正済み／戦術リセット設定の分離・経験済み職の前提免除は現行仕様／カルマ下限1は現行仕様を維持（明示決定）",
        "note": "31d5c1bで成功時にREMOTE_ADDRを保存するよう修正した。能力減少値と職業Lv遷移は一致する。現在の既定値はmaster_tactics_enabled=1・reset_tactics_on_job_change=1でVer2のmaster_tac=1と同じ結果になる。config.pyの説明どおり、現行は他職戦術を候補へ加えるかと転職直後に選択中戦術を解除するかを独立して設定する。保存済み熟練度がLv1以上の経験済み職へは、能力値・カルマ・必要マスター職を再確認せず転職できる。Ver2は負値だけを1へ戻すためカルマ0を許容する一方、現行は0も1へ戻す。再精査で履歴根拠がないことを確認したうえで、カルマ最低値を1とする現行仕様をユーザーが明示決定した。",
    },
    {
    ...
    },
    {
        ...
        "difference": "宿泊成功時の接続元ホスト保存と、対象IDの本人照合・POST限定が欠けていた",
        "intent": "不具合を修正済み",
        "note": "宿代Lv×10、HP全快、王者HP全快、boss_flagの10への復帰は一致。成功時にREMOTE_ADDRも保存する。ログイン済み判定だけではフォームのid差し替えを防げなかったため、他の状態変更入口と同じrequire_owner照合を加え、login.pyのshopルートをPOST+CSRF限定にした。",
    },
    {
        "file": "cgi_py/bank.py",
        ...
        "intent": "不具合を修正済み",
        "note": "d803be1でASCII数字だけを受理し、預金上限超過分はVer2の画面案内どおり国への寄付として所持金から差し引き、預金へは上限までだけ加算するよう修正した。Ver2のbank_sell本体は上限を越えて加算しており、案内文と矛盾していた。",
    },
    {
        ...
        "intent": "接続元と保管順は現行仕様を維持（明示決定）／破棄確認は不具合を修正（明示決定）",
        "note": "3種別の上限は8で一致。現行は選択品を配列から取り出し、旧装備を末尾へ追加し、着脱時にREMOTE_ADDRを保存しない。Ver2は選択位置を旧装備で置換し、着脱時に接続元を保存する。再精査で差異の事実と履歴根拠の不在を確認したうえで、接続元を保存しないことと末尾追加方式をユーザーが明示決定した。破棄はCSRF付きPOSTの直前に共通確認モーダルを表示する。",
    },
    {
    ...
    },
    {
        ...
        "intent": "処理移行・防具2181・売却時接続元は現行仕様を維持（明示決定）",
        "note": "購入・売却額は両版とも価格の2/3、購入時のREMOTE_ADDR保存と防具倉庫上限8は一致する。Ver2は売却時にもREMOTE_ADDRを保存するが、現行は保存しない。再精査で保存省略の履歴根拠がないことを確認したうえで、売却時も接続元ホストを更新しない現行仕様をユーザーが明示決定した。2165・2183の通常長音符への統一と、皇帝用2181の販売追加も既存判断として現行維持する。",
    },
    {
        ...
        "intent": "処理移行・売却時接続元は現行仕様を維持（明示決定）",
        "note": "購入・売却額は両版とも価格の2/3、購入時のREMOTE_ADDR保存と装飾品倉庫上限8は一致する。Ver2は売却時にもREMOTE_ADDRを保存するが、現行は保存しない。再精査で保存省略の履歴根拠がないことを確認したうえで、売却時も接続元ホストを更新しない現行仕様をユーザーが明示決定した。職別品揃えと補正は一致し、85・87の率補正はd7105f6のVer1準拠調整として既存判断を維持する。",
    },
    {
        ...
        "intent": "一般掲示板は意図的な追加／私信は機能廃止（明示決定）",
        "note": "ログイン本人だけが200文字以内・禁止語なしで投稿でき、最新100件を新着順に共有JSONへ保持する。login.pyは全POSTをルーティング前にCSRF検証し、投稿後はPRGリダイレクト、共有ロックを用いる。243317cで郵便局を街の一般掲示板へ統合し、5f18bc5で私信の変換・既存message/message_sent保存も廃止した。Ver2の私信・送信箱・受信拒否・友人登録を掲示板へ置き換えたものではない。",
    },
    {
        ...
        "intent": "画面・安全化は意図的／待機20秒は現行仕様を維持（明示決定）／街へ戻る際の進行リセットはVer2準拠（明示決定）",
        "note": "戦闘回数0ではモンスター・幻影・異世界・道場・武道会・レジェンドを表示段階で止め、クラス、宿代Lv×10、称号段階は一致する。待機はVer2の30秒に対して現行設定値20秒であり、二次精査でユーザーが20秒維持を明示決定した。Ver2はboss_flagが挑戦開始値以外ならレジェンド入口から挑戦させない。5f4bdacで中断時の進行リセットを復元し、9951abbで明示中断だけでなく街へ戻った時点で途中のboss_flagを常に10へ戻すよう補正した。戦闘結果画面からの連戦は街を経由しないため、同じ階層内では継続できる。",
    },
    {
        ...
        "note": "命中=dex/10+51（上限150）、回避=agi/20（上限50）、必殺=karma/15+10+job_level（上限75）、装備補正、現在職のクラス表示、100文字・禁止語制限、更新時のREMOTE_ADDR保存は確認した。Ver2のsite/urlだけは現行スキーマから除外し、現行はアイコン番号を設定済み画像へ制限する。4fbcbc3で現職はchara.job_level、転職済み職はsyokuから熟練度を取り、全職業を表示するよう補正した。能力名の括弧内説明は実装との対応が曖昧だったため、既決方針で非表示にしている。",
    },
    {
        ...
        "intent": "時間切れ・敗北経験値は現行仕様を維持／移行・安全化は意図的",
        "note": "対人時間切れwin=3は経験値だけを得て、王者交代・賞金・防衛連勝・次回賞金を発生させない。Ver2は時間切れ時に王者の防衛側更新を行う。03a81a1は時間切れを相打ちと分離し、王者交代等を抑止する目的を実装・コミット説明で確認した。敗北EXPはVer2の相手Lvそのままではなくmin(相手Lv, 自分Lv×10)に制限する。07fb32fは高レベル王者への連続挑戦だけで経験値を稼ぐ経路を防ぐ目的でこの上限を導入している。通常勝利・相打ちwin=2の王者交代、敗北時の半額、boss_flagと修行回数の復帰、レベルアップ後に新王者と戦闘後HPを確定する順は確認した。",
    },
    {
        ...
        "note": "名前の完全一致検索、本人との対戦拒否、待機20秒、対人BattleSimulator、経験値・所持金・戦績・待機時刻を保存しない点は確認した。Ver2の候補一覧はキャッシュ済みRANKINGを使う一方、現行は保存済み全ユーザーを直接列挙する。bebbe32でVer2の街画面と同じく初回チャンピオン戦後に解放する導線を復元し、現行はbattle_count>0を入口・直接POSTの双方で検査して画面制限の迂回も拒否する。",
    },
    {
        ...
        "difference": "直接実行時の解放・異世界Lv制限、20秒待機、JSON重み抽選へ移行。幻影・異世界でも後続モンスター技フックを呼ぶが、全IDが空処理で実効差はない",
        "intent": "敗北EXPは不具合を修正済み（明示決定）／入口検証・抽選移行は意図的／待機20秒と後続技フックは現行仕様を維持（明示決定）",
        "note": "Ver2の重複行数と現行weight合計は初級193・中級169・上級45・最上級121・異世界95で全て一致する。通常・幻影・異世界の回数消費、勝敗時の所持金、戦績、boss_flag復帰、レベルアップ後のHP回復、接続元保存は確認した。現行はbattle_count>0と異世界Lv300を実行側でも検査する。待機はVer2の30秒に対して現行設定値20秒であり、二次精査でユーザーが20秒維持を明示決定した。Ver2の敗北EXPは1で、初期Python移植から0だった現行を再精査のうえユーザー明示決定により1へ復元した。Ver2は通常修行だけmons_atowazaを呼び、幻影・異世界では呼ばない。現行は全モードで呼ぶが、mons_0〜mons_22の同フックは全て空実装のため実効差はない。",
    },
    {
        ...
        "intent": "階層値検証は不具合を修正済み／連戦・クリア後再挑戦は現行仕様を維持（明示決定）",
        "note": "各階層は両版とも11体で、boss_flag 10から1へ減らして0で階層クリアする。Ver2の通常画面はboss_flagが10以外なら入口を隠すが、legend.cgi自体は進行値を検証しないため、直接POSTでは0番目の行を参照する不整合な再入場が可能だった。7754ecaは現行仕様として連戦・最終クリア後の再挑戦を導入し、開始値10へ戻して同階層を正しく最初から再挑戦可能にした。明示中断時も5f4bdacで開始値へ戻す。攻略者向けの全体メッセージと「新しい称号」の表示は、c6ff504でtitle_idが実際に上がる初回攻略時だけに限定した。勝利・引分・敗北の経験値、所持金、battle_count、battle_limit、title_id、レベルアップ後HP回復、接続元保存を照合した。実行はlogin.pyがPOST・CSRFを検証する。範囲外・数値外のboss_fileは第1階層へフォールバックしていたため拒否へ修正した。",
    },
    {
        ...
        "intent": "進行分岐・20件履歴は現行仕様を維持（明示決定）／履歴変数名・ロビー表示は不具合を修正済み",
        "note": "参加者は両版とも24時間ごとにレベル上位3人を固定し、対戦時はその能力スナップショットと最新装備を組み合わせる。相手順は3位→2位→1位で一致し、ラウンド番号を現行はサーバー側でも照合する。Ver2はsentoukeka後に結果を問わずboss_flagを減らすため、相打ちは勝利扱い、時間切れ・敗北も次戦へ進行する。現行は勝利だけ進行し、相打ち・時間切れは進めず、敗北も開始状態を保つ。03a81a1は時間切れを相打ちと分離して進行を抑止した実装・説明の根拠である。相打ち・敗北の非進行も再精査でVer2の無条件進行に合理的な根拠がないことを確認し、勝利のみ進行する現行仕様をユーザーが明示決定した。battle_limit補充、経験値、所持金、レベルアップ後HP回復、接続元保存を確認した。Ver2の制覇履歴はtenka_su=3で通常4件保持し、現行は設定値20件を保持する。途中進行時に第1回戦を再送するロビー導線と、実装と異なる5名・順位交換の説明を修正した。",
    },
    {
    ...
    },
    {
        ...
        "note": "レベル降順、20件単位、削除期限、勝率、能力値・装備補正・命中/回避/必殺・称号・マスター職、画像ID一覧を照合した。dababeaはVer2と同じ7段階の職業クラスを復元し、負数・過大なshtmは安全に有効範囲へ正規化する。6ec242eは性別・所持金を他者詳細へ復元した。現行はキャッシュの必須キーを検証し、外部ホームページURLだけは現行スキーマから除外する。バーはCSS幅100%へ正規化する表示差で、計算値自体は一致する。",
    },
    {
        ...
        "intent": "Lv基礎ダメージ・敵行動停止は不具合を修正済み／回復上限は現行仕様を維持（明示決定）",
        "note": "通常・幻影・異世界・レジェンドはLv×(rand(5)+1)、対人は双方Lv×(rand(3)+1)を職業式と武器ATKへ加算する。全31職の能力参照を再確認し、上級職のカルマはall_stats・職24・職25で固定加算とする。必殺技・モンスター技の個別効果はskills.md・monster_skills.mdで別途照合済み。双方の行動後にHPを同時精算し、倒す側・倒される側とも同ターンに成立した相手のダメージを受ける。各回復は最大HPで止めてから被ダメージ・自傷を差し引く。",
    },
    {
        "file": "cgi_py/chocorank.py / templates/chocorank.html",
        "v2_source": "旧版_ver2/chocorank.cgi:ranking,rank / chocolog/*.cgi / rireki.cgi",
        "scope": "飼育中チョコボ抽出、24時間キャッシュ、勝数・訓練数・7能力・獲得賞金の各Top 10、能力ランク、重賞制覇履歴、牧場への復帰",
        "difference": "JSON・ロック・テンプレートへ移行。現行テンプレートが別形式のranksを参照し、集計済みのrankings/rirekiを表示していなかった",
        "intent": "テンプレート契約と能力ランク境界は不具合を修正済み／JSON保存・24時間キャッシュ・数値併記は意図的",
        "note": "Ver2の10部門（勝数、訓練数、筋力、スタミナ、粘り、落ち着き、闘争心、賢さ、反射神経、獲得賞金）を各10件・同じ並びで表示する。退役済みの空データはcommon.choco_loadで対象外とし、賞金は両版とも保存値の100倍を表示する。能力画像はVer2の厳密な>100、>200、>400、>600、>800、>1000境界に戻す。887dd03でテンプレートへrankings/rirekiを接続し、キャッシュ版を上げて旧ランク番号を即時再構築した。590e7a9で閲覧をログイン本人に限定する。現行は画像だけだったVer2に実数値とキャッシュの最終更新時刻も併記し、重賞履歴はr1〜r22を全レース名と○/−で表示する。",
    },
    {
        "file": "cgi_py/chocofarm.py / cgi_py/crace.py / templates/chocofarm.html",
        "v2_source": "旧版_ver2/chocofarm.cgi / crace.cgi / choco-farm.pl",
        "scope": "本人用牧場、チョコボ・王者表示、能力ランク、クラス・体調、訓練・一般レース・G1/G2開催導線、重賞タイトル、海外重賞解放、復帰フォーム",
        "difference": "JSON・CSRF・セッション本人確認・テンプレートへ移行。本人以外の牧場表示と海外重賞直接POSTの条件検証が欠け、重賞のG1/G2色情報をテンプレートへ渡す前に破棄していた",
        "intent": "本人確認・直接POST検証・重賞色分け・自己チョコボ詳細の表示量は不具合を修正済み／待機20秒と許可境界は現行仕様を維持（明示決定）／殿堂レースは5頭のランダム抽選へ調整（明示決定）",
        "note": "クラス境界、体調、一般レースの勝数範囲、G1の40回・G2の60回周期、性別限定最終枠は一致する。race_dendoはVer2のcrace2.cgiをcrace.pyへ統合した経路で、殿堂JSONをライバル表として使う。Ver2は殿堂一覧で5頭を指定してcrace2.cgiへ渡す。現行は入力欄を持たないまま、ユーザー明示決定により自チョコボに加えてdenchoco.jsonから5頭をランダム抽選する。牧場の能力画像はVer2もint(能力/100)であり、ランキング画面の厳密な閾値判定とは別仕様のため変更しない。4c43c19は本人以外の牧場表示と、開催枠・性別枠・海外重賞3勝条件の直接POSTを実行側でも拒否する。989b78fはレース開始へのCSRF検証、5cde55fは重賞記録をユーザーロック下で再読込・マージして保存するための修正である。現行は待機終了をちょうど設定秒で許可するが、Ver2は30秒を超えた時点で許可する。二次精査でユーザーが現行20秒維持を明示決定したため、現行の許可境界も維持する。自己チョコボの画像・性別・血統・訓練数・7能力ランクと実数値を表示へ復元し、ステータスカードを2カラム全幅、訓練・お世話・アドバイス・関連施設を後続の2カラムへ再配置した。",
    },
    {
        "file": "cgi_py/morifarm.py / templates/morifarm.html",
        "v2_source": "旧版_ver2/morifarm.cgi / choco-farm.pl / chocobofile.cgi / chocoboos.cgi / chocoboms.cgi",
        "scope": "野生チョコボ探索・購入、お見合い候補・配合、血統/能力/性別/タイプ乱数、命名、休養、引退と候補リスト、重賞履歴、本人認証・CSRF",
        "difference": "JSON・テンプレート・本人認証へ移行。状態変更フォームはCSRFトークンを出力していたが、処理側の検証が欠けていた。配合のprebirth乱数はVer2の合算後切捨てを個別切捨てへ誤移植していた。世代をまたぐ重賞履歴は現行で持たない",
        "intent": "CSRF検証、空名の直接配合拒否、prebirth乱数、配合時の乱数分布、引退金と誤認させる表示、同一育て親候補の追加は不具合を修正済み／候補上限100件と重賞履歴リセットは現行仕様を維持（明示決定）",
        "note": "野生購入時の初期値、候補数1〜5、血統表、能力上限・初期能力、休養の5,000G・200〜499回復・max加算は一致する。Ver2のprebirthはint(rand(相手祖先合計)+rand(自分祖先合計))であり、合算してから切り捨てる。配合時の性別と突然変異画像・タイプは、Ver2のint(rand(1.9))・int(rand(7.999))・int(rand(5.1))へ修正し、端数を含む分布を再現する。候補上限100件と肥大化させず既存枠を置換する処理は13ef4fbで明示されており維持する。Ver2どおり、同じ育て親の引退個体を通常候補とは別に25%で追加表示し、表示上も区別する。メス親時のVer2は相手決定前の父名を使う不具合があり、現行の選択相手名を使う実装を維持する。引退でキャラクター所持金は増えず、候補としての想定引取額だけを記録する。Ver2は現役チョコボを作り直しても重賞履歴を保持する一方、現行は野生購入・配合時にchoco_g1を空にする。二次精査でユーザーが世代交代時のリセット維持を明示決定した。",
    },
    {
        ...
        "difference": "JSON・テンプレート・本人認証へ移行。現行は一覧を認証済み利用者に限り、同一ID・同名の更新では元の並び位置を維持する。テンプレートは所有者の未定義参照、能力等の表示欠落、maxmaxの年齢誤表示、登録完了メッセージ未表示があった",
        "intent": "CSRF検証と殿堂詳細表示の不備を修正済み／一覧の公開範囲と同一登録更新時の並び順は現行仕様を維持（明示決定）",
        "note": "Ver2はIDとチョコボ名が一致する登録も先頭へ移動し、新規も先頭へ追加する。現行は一致時に同じ位置で上書きするため、更新時の並び順が異なる。登録時はVer2どおり重賞数を問わず、名前付きの現役チョコボを対象とする。一覧はVer2ではログイン不要だが、現行はa533b17のIDOR対策でrequire_ownerを無条件に呼ぶため認証済み利用者に限られる。二次精査でユーザーが一覧のログイン必須と元位置更新の維持を明示決定した。Ver2の殿堂一覧は画像・性別・タイプ・父母・戦績・訓練・能力ランク・重賞を表示しており、現行でも復元した。引退年齢として表示していたmaxmaxは年齢ではないため廃止し、想定引取額を表示する。現行は登録時点のトロフィーを殿堂データにスナップショットするため、世代交代後も記録が混ざらない。",
    },
    {
        "file": "cgi_py/farmrace.py / templates/farmrace.html",
        "v2_source": "旧版_ver2/farmrace.cgi / choco-farm.pl / farmwinner.cgi",
        "scope": "チョコボ王者への挑戦条件、20ターンの移動・スタミナ・写真判定、勝敗報酬、王者交代/防衛、能力上限・老衰、保存、レース結果表示",
        "difference": "JSON・本人認証・構造化ログへ移行し、外部サイト情報を王者データから除去。終盤のスタミナ消費とラストスパート判定がVer2からずれていた",
        "intent": "終盤計算と結果ログの不備を修正済み／待機20秒・所持金上限・外部サイト情報の廃止は現行仕様を維持（明示決定）",
        "note": "体力400以上、同一王者の再挑戦拒否、20ターン、写真判定、勝利時のrun/win・cmax+80・各能力+1〜6・賞金、敗北時のrun・cmax+20・各能力+1・防衛連勝、寿命-200、老衰・能力/総合上限の順序は一致する。5f1e126で終盤をVer2どおり補正前移動量からスタミナ消費を再計算し、両者の闘争心判定をrand(能力×1.5)に戻した。989b78fは開始時CSRF検証、9cd9d14は旧外部サイト情報を読み込み時点で破棄して再保存しない変更である。待機はVer2の30秒に対して現行設定値20秒であり、二次精査でユーザーが20秒維持を明示決定した。結果ログは簡略化を維持しつつ、各ターンの残り距離を表示する。",
    },
    {
        "file": "cgi_py/ctrain.py / templates/ctrain.html",
        "v2_source": "旧版_ver2/ctrain.cgi / choco-farm.pl",
        "scope": "7種訓練の選択、20回成功/失敗、能力と副作用、個別/総合上限、老衰、寿命、待機、保存、結果表示、本人認証・CSRF",
        "difference": "JSON・本人認証・構造化ログへ移行。無効modeの暗黙実行を拒否し、20秒待機へ設定化。無名の子チョコボを直接訓練でき、負能力副作用の順序がVer2からずれていた",
        "intent": "無名個体拒否、負能力処理順、CSRF検証を不具合として修正済み／無効mode拒否と待機20秒は現行仕様を維持（明示決定）",
        "note": "訓練種別と7能力の増減、20回・成功率75%、知力訓練の固定問題、成功のみ/混在/失敗のみの限界突破+3/+5、訓練数・寿命-50・cmax+5、老衰、総合上限時の最大能力限界+2確率1/3は一致する。444bc30は無名個体の実行拒否・CSRF検証と、Ver2どおり老衰・個別上限の後に負能力を1へ戻し最大能力限界を-5する順序を復元した。Ver2は待機30秒で無効mode時にも処理へ進む。現行の無効mode拒否は意図を確認でき、待機20秒は二次精査でユーザーが維持を明示決定した。",
    },
    {
        "file": "login.py / templates/others.html",
        "v2_source": "旧版_ver2/login.cgi / regist.pl",
        ...
        "note": "Ver2は成功・失敗とも最大15件をloginlogへ保存し、失敗時は入力パスワードまで記録する。現行の変換器はlogin_logへpass・host・time・failedを移すが、login.pyはlogin_log_registを呼ばないため以後更新されない。平文パスワードを含むVer2の履歴機能は廃止で確定し、移行済みの過去値は互換データとして残す。7b6086fは新規登録とログインのID検証を統一し、2afe78eは再ハッシュ・testのlast_time更新をユーザーロック内で再読込・認証・一括保存して並行中の進行を上書きしないようにした。438e059は状態変更ルートのPOST限定とCSRFを追加した。",
    },
    {
        "file": "others.py / templates/others.html",
        "v2_source": "旧版_ver2/others.cgi / regist.pl:read_winner",
        "scope": "公開トップ、ログイン・新規登録・テストプレイ、記憶Cookie、CSRF、現役一覧、人間/チョコボ王者、能力・率・クラス、案内・公開リンク、メンテナンス",
        "difference": "フレーム内のShift_JIS直書き画面を単一UTF-8テンプレートへ移行。平文ID/pass Cookieとチャンプ・前王者の外部サイトURLを廃止し、暗号化セッション、CSRF、チョコボ王者詳細、ログイン状態の再開導線を追加",
        "intent": "現行仕様を維持",
        "note": "人間チャンプのクラス（job_level÷10）、勝率、能力バー、命中・回避・必殺率の上限、テスト用アカウントのlast_timeから600秒判定は一致する。現役一覧もVer2と同じ最終アクセスから有効時間内を表示し、現行は120秒設定・原子的なread-modify-write・名前のHTMLエスケープとIDのURLエンコードを追加する（a4b27ae）。外部サイトURLはdata_schema/file_opsが読込・保存時に破棄し、平文パスワードは暗号化セッションのID再開導線だけへ置換する。能力値と装備補正・チョコボ王者のクラス/戦績/能力は現行でも表示する。",
    },
    {
        ...
        "intent": "現行仕様を維持",
        "note": "a4b27aeで管理パスワードをURL・hidden値から除去し、初回POSTの定数時間比較、以後の暗号化セッション認可、状態変更CSRFへ移行した。マスターはID一意性・職業参照・数値型を検証し、職業は配列順依存のため削除不可、倉庫追加はマスター実在・種別上限を確認する。個別/放置削除はprotected_user_idsを除外し、8df5bb3で固定バックアップを優先する保護ユーザー復元と、バックアップがないtestの初期化を追加した。e4c9812の日次バックアップ復元はmaintenance_modeを必須にし、復元前のセーブを退避する。Ver2の管理ログは認証失敗時も入力パスワードを保存していたため、現行では復元しない。",
    },
    {
        "file": "sub_def/common.py",
        "v2_source": "旧版_ver2/regist.pl / choco-farm.pl",
    ...
    },
    {
        ...
        "v2_source": "旧版_ver2/charalog/<ID>.cgi / item/<ID>.cgi / change_data/convert_all.py",
        "scope": "chara・統合ユーザーデータ・装備・アクセサリー能力/率のキー順、旧キー移行、外部サイト情報除去、未知キー保持",
        "difference": "Ver2の固定列・分割レコードを名前付きJSONへ移行し、保存時の表示順を定義する。title/unused30をtitle_id/tactic_idへ読み替え、site/urlを廃止する",
        "intent": "現行仕様を維持",
        "note": "CHARA_KEY_ORDERはVer2の意味上の列（能力8種、職業、経験値、所持金、戦績、待機、称号、戦術）を現行キー順で固定する。order_user_dataは未知の追加キーを末尾へ保持し、equipment/item内の武器・防具・アクセサリーとbonus8種を安定順にする。値の変換はtitle/unused30とsite/url除去に限定し、その他の互換データを破棄しない。",
    },
    {
        "file": "sub_def/crypto.py",
        "v2_source": "旧版_ver2/login.cgi:set_cookie,log_in / regist.pl:decode",
        "scope": "セッション暗号化・署名・期限・破棄、CSRF生成/検証、パスワード新旧形式のハッシュ・照合・再ハッシュ判定、Cookie属性",
        "difference": "Ver2のIDと平文パスワードを60日Cookieへ入れる方式と平文照合を、署名付き暗号化セッション・30分期限・HttpOnly・CSRF・PBKDF2-SHA256へ移行",
        "intent": "現行仕様を維持",
        "note": "セッションCookieはHMACを定数時間比較してから復号し、created_atとsession_expiryで失効させる。CSRFはセッション内の乱数をフォームsと定数時間比較する。verify_passwordは新形式、旧固定ソルト、平文を受け付け、login.pyは成功時だけ新形式へ再ハッシュする。Secure属性はHTTP対応のため未設定であり、HTTPS化時の運用設定として別管理する。",
    },
    {
        "file": "sub_def/utils.py / templates/base.html / templates/error.html",
        "v2_source": "旧版_ver2/regist.pl:header,footer,error",
        "scope": "テンプレート描画、UTF-8/キャッシュ制御ヘッダー、セッションCookie/CSRF注入、例外時の応答、エラー終了、302リダイレクト、トースト通知",
        "difference": "Ver2のShift_JIS直書きheader/footer/errorを、Jinja2の自動エスケープ付きUTF-8テンプレートへ移行。セッション内の一回限りトーストと開発CGI向けmeta refreshを追加",
        "intent": "現行仕様を維持",
        "note": "render_templateは既存セッションのCSRFトークンを維持してフォームへ注入し、呼出元の重複Set-Cookieを除いて1本だけ発行する。テンプレート例外の詳細はstderrだけへ記録し、HTTP応答は汎用文に留める。redirect/show_error/redirect_with_flashはいずれも出力後に終了し、戻る操作や別タブでフォームを直ちに失効させない。",
    },
    {
        "file": "sub_def/backup.py",
        "v2_source": "旧版_ver2/admin.cgi:save_chara,save_del,del_noplay / save_log.cgi",
        "scope": "日次バックアップ作成、manifest検証、世代削除、一覧、復元前退避、選択復元、メンテナンス要求、バックアップ排他、保護ユーザー用固定バックアップとの役割分担",
        "difference": "Ver2の放置削除除外用save_logと手動復元運用に対し、現行はsave_data全体の日次スナップショット・世代管理・検証済み選択復元を追加",
        "intent": "現行仕様を維持",
    ...
    },
    {
        ...
        "intent": "現行仕様を維持",
        "note": "7b6086fでvalidate_user_idを新規登録とログインで共用し、保存先探索へ形式外IDを渡さない。IDはVer2どおり4〜8文字の半角英数字。ゲーム用パスワードは過去判断どおり4〜8文字のASCII記号を許可し、Ver2の英数字限定へ戻さない。キャラクター名は2〜16文字・cp932互換、チョコボ名は20文字以内・cp932互換と制御文字拒否を現行の表示/保存保護として維持する。変更用単語はVer2同様に空値だけを拒否する。",
    },
    {
    ...
    },
    {
        ...
        "intent": "現行仕様を維持（待機20秒は明示決定）",
        "note": "$turn=150、$sentou_limit=9999、$yado_dai=10、$lv_up=300、$master_tac=1、各装備所持上限8、$boss=10、メッセージ保存20件は対応する現行値を維持する。対人/レース/天下一はpvp_race_cooldown_seconds、モンスター/伝説/チョコボ訓練はtraining_cooldown_secondsを各実行画面が参照する。Ver2は共通30秒、現行は共通20秒で、二次精査でユーザーが20秒維持を明示決定した。盗み上限、異世界/モンスター段階、JSONマスター、画像/職業ID辞書、バックアップ・セッション・自動ロック回復、チョコボお見合い候補の性別ごと100件は現行固有の仕様として維持する。番号付きマスターと初期職業IDは起動時に整合性を検証する。",
    },
    {
        "file": "dev_server.py",
    ...
    },
    {
        ...
        "difference": "Ver2のsite/url入力・公開確認を廃止し、現行は番号付き画像辞書・初期職業辞書、Jinja自動エスケープ、CSRF、パスワード記号許可を追加。修正フォームはVer2同様に入力を再表示するが、パスワードだけは再入力を求める",
        "intent": "現行仕様を維持",
        "note": "作成確定はmode=make_endと全登録値・CSRFをPOSTし、サーバー側の再検証と排他登録へ渡す。c8ae321は確認画面の並行送信でもID・名前・ホスト重複確認から保存までを同じロックで直列化した。Ver2の修正フォームもmodeなしで初期画面へ戻るが、旧画面は受信値をvalue属性へ再表示していた。bd6a3b8で現行も修正フォームからID・名前・変更用単語・性別・画像・職業を再表示するよう補正した。一方、パスワードはHTMLへ再出力せず再入力を求めるため、Ver2の平文再表示より安全な現行仕様とする。",
    },
    {
        ...
        "intent": "現行仕様を維持",
        "note": "has_wordsで初回単語登録と変更フォームを分け、いずれもPOSTのid/mode/入力値を送る。bd6a3b8でテンプレートが送るsをpassset/passchanの入口で検証するよう修正し、表示だけの画面遷移には検証を要求しない。変更用単語は初回設定後に変更不可というVer2の運用を維持し、完了画面で単語を一度だけ表示する。65b805bで新パスワードの許可文字を登録時と統一し、変更時のcharaと変更情報の両方へREMOTE_ADDRを保存するよう補正した。Ver2は平文の履歴行を先頭追加するが、現行はハッシュ済みの最新1件だけを原子的に保存する。",
    },
    {
        ...
        "intent": "現行仕様を維持",
        "note": "選択画面はtactic_idに一致するラジオを初期選択し、mode=senjutu_henkou、senjutu_no、CSRFをPOSTする。確定時はロック後に最新の職業/熟練度で候補を再構築して再検証する。bd6a3b8でテンプレートが送るsをsenjutu_henkou入口で検証するよう修正し、結果画面からの表示遷移は状態変更しない。",
    },
    {
        ...
        "intent": "現行仕様を維持",
        "note": "転職フォームは、未経験職には能力・熟練度前提を満たす候補、経験済み職には前提を免除した再転職候補を表示する。確定時にも未経験職の前提を再検証する。mode=tensyoku_changeではテンプレート送信のsを検証するよう修正済み。結果画面の神殿・街へのPOSTは表示遷移だけで状態変更しない。",
    },
    {
        "file": "templates/sts.html / templates/sts_result.html / cgi_py/sts.py",
        "v2_source": "旧版_ver2/sts.cgi",
        "scope": "詳細ステータス、職業熟練度、装備/称号/能力表示、画像・クリティカル台詞変更、完了画面、表示設定、CSRF",
        "difference": "Ver2の表形式ステータスを現行はID付きマスター・能力バー・クラス表示・ブラウザ内表示設定へ移行。コメントと画像の保存を本人確認・再読込・CSRFで保護",
        "intent": "現行仕様を維持",
        "note": "st_buyは画像番号とコメントをサーバー側で検査し、ロック後の最新データへ保存する。e851d64でテンプレートが送るsをst_buy入口で検証するよう修正した。トースト位置と数値表記はブラウザlocalStorageだけを更新し、サーバー上のキャラクターデータを変更しない。",
    },
    {
        "file": "templates/bank.html / cgi_py/bank.py",
        "v2_source": "旧版_ver2/bank.cgi:bank_shop,bank_buy,bank_sell",
        "scope": "預金・引出、全額操作、上限超過分、残高/可能額表示、完了通知、CSRF",
        "difference": "Ver2の画面内メッセージを現行はトーストとリダイレクトへ移行。銀行・所持金の上限を設定化し、上限超過分を寄付として消滅させる現行仕様を明示。CSRFを追加",
        "intent": "現行仕様を維持",
        "note": "bank_sell/bank_buyは入力を千Gold単位として検査し、保存前に所持金・預金・各上限を再確認する。上限超過の預入分は既決定どおり国への寄付として手持ちから消滅する。2つの状態変更モードでテンプレート送信のsを検証するよう修正済み。",
    },
    {
        "file": "templates/souko.html / cgi_py/souko.py",
        "v2_source": "旧版_ver2/souko.cgi",
        "scope": "武器・防具・装飾品の装備、取り外し、破棄、倉庫上限、現在装備/保管品表示、CSRF",
        "difference": "Ver2の種別ごとの保存ファイルを現行は統合JSONへ移行し、装飾品の能力説明を共通整形。本人確認・原子的な複数区画保存・CSRFを追加",
        "intent": "保管順・接続元は現行仕様を維持（明示決定）／破棄確認は不具合を修正（明示決定）",
        "note": "weapon/armor/accessoryのremove・equip・delete全9モードは、テンプレート送信のsを状態変更前に検証するよう修正済み。Ver2は着脱時に接続元を保存し、選択位置を旧装備で置換する。現行は接続元を保存せず、選択品を除去して旧装備を末尾へ追加する。再精査で履歴根拠がないことを確認したうえで、この現行方式と破棄前の共通確認モーダルをユーザーが明示決定した。容量上限は取り外し時に確認する。",
    },
    {
    ...
    {
        "file": "templates/select_battle.html / cgi_py/select_battle.py",
        "v2_source": "旧版_ver2/select_battle.cgi",
        "scope": "名前検索、登録者一覧からの相手選択、道場模擬戦、解放条件、待機時間、結果画面への引継ぎ",
        "difference": "Ver2の個別HTMLをJinjaテンプレートとmonster_result.html再利用へ移行。初回チャンプ戦後の解放条件と20秒待機を現行設定で適用",
        "intent": "現行仕様を維持",
        "note": "battleはBattleSimulatorをメモリ上で実行するだけで、経験値・所持金・戦績・HP・last_timeを保存しない。従って検索、一覧表示、模擬戦開始はいずれも状態変更ではなくCSRF検証対象にしない。直接battleを送ってもチャンプ挑戦前のキャラクター、自分自身、存在しない相手はサーバー側で拒否する。",
    },
    {
        "file": "templates/monster_result.html",
        "v2_source": "旧版_ver2/monster.cgi / legend.cgi / battle.cgi の戦闘結果出力",
        "scope": "通常/幻影/異世界/伝説/チャンピオン/道場の結果見出し、勝敗・報酬・レベルアップ文、ターンログ、レジェンド継続/中断導線と待機表示",
        "difference": "Ver2の各CGI内HTMLを共通Jinjaテンプレートへ統合し、モード名・HPバー・レジェンド待機ボタンを追加",
        "intent": "現行仕様を維持",
    ...
    },
    {
        ...
        "difference": "Ver2の簡易一覧をカード・能力ランク・重賞表示へ移行し、殿堂データをJSON化。現行は一覧をログイン済み利用者に限定し、同一ID/名前の更新を元位置で行う",
        "intent": "表示内容・CSRFは意図的／一覧公開範囲と更新時の並び順は現行仕様を維持（明示決定）",
        "note": "一覧は読み取り専用でCSRF不要。mode=dendoだけが殿堂リストを更新するためCSRF検証を行う。Ver2は同一ID/名前の更新も先頭へ移動し、未登録も先頭追加するが、現行の同一登録は元位置で更新する。Ver2の一覧は公開だが、現行はa533b17で状態変更を守るために追加したrequire_ownerが表示にも掛かる。二次精査でユーザーがこの一覧制限と元位置更新の維持を明示決定した。登録条件はVer2どおり命名済みの現在のチョコボであり、重賞3勝は要求しない。",
    },
    {
    ...
    {
        "file": "templates/ctrain.html / cgi_py/ctrain.py",
        "v2_source": "旧版_ver2/ctrain.cgi",
        "scope": "訓練20回の結果ログ、成功・失敗数、能力/寿命/限界の変動、再訓練・牧場復帰導線、待機、CSRF",
        "difference": "Ver2の画面内HTMLを結果テンプレートへ移行し、現行は結果画面から訓練種別を選び直して再訓練できる。状態変更POSTにはCSRFを追加",
        "intent": "現行仕様を維持",
        "note": "成功率、20回のログ、各訓練の増減・副作用、訓練数+1、寿命-50、総合/個別上限、老衰判定はVer2と照合済み。訓練開始は常に保存を伴うため、mode=race0〜race6を処理側で検証したうえで送信sを検証する。牧場へ戻るPOSTは表示遷移のみで、再訓練フォームとともに最新のCSRFトークンを渡す。",
    },
    {
        "file": "templates/crace.html / cgi_py/crace.py",
        "v2_source": "旧版_ver2/crace.cgi",
        ...
        "intent": "通常・重賞の5頭立ては現行仕様を維持／殿堂レースの5頭ランダム抽選は明示決定",
        "note": "通常・重賞は自チョコボを含む5頭立てを維持する。殿堂レースはユーザー明示決定により、自チョコボに加えて殿堂チョコボ5頭をランダム抽選する6頭立てとする。実況文、勝敗文、所持金、能力/寿命/上限、訓練・出走回数、重賞のタイトル記録と通知は実行処理で照合済み。テンプレートは各区間の出走頭数に応じて残距離から位置・順位を自動再生し、再生後も全実況ログを読み返せる。出走名は自動エスケープし、サーバー組立ての実況/結果文だけをsafe表示する。レース実行は常に保存を伴うため、開始時に送信sを検証し、一般/重賞/殿堂の開催・性別・戦績条件もサーバー側で再検証する。重賞履歴も最終ユーザーロック内で既存値とマージして保存する。",
    },
    {
        "file": "templates/farmrace.html / cgi_py/farmrace.py",
        "v2_source": "旧版_ver2/farmrace.cgi",
        "scope": "チョコボ王者決定戦の挑戦者/王者、レース実況、勝敗・賞金・王者更新、能力変動、牧場復帰、CSRF",
        "difference": "Ver2の2頭によるブラウザ内レース実況を、現行は計算済み区間データを使う現代ブラウザ対応のコース再生と結果レポートへ移行。JSON保存・本人確認・CSRFを追加",
        "intent": "現行仕様を維持",
        "note": "Ver2どおり挑戦者と王者の残距離から20ターン以内で勝敗/写真判定を決め、勝利時の新王者登録・防衛時の連勝更新・報酬・能力/寿命/上限を実行処理で照合済み。テンプレートは2頭の各区間残距離を位置・順位として自動再生し、全実況ログも残す。挑戦開始は保存を伴うため、送信sを検証し、本人・命名・体力・待機・現王者本人の再挑戦をサーバー側で拒否する。",
    },
    {
        "file": "html/main.js",
        "v2_source": "旧版_ver2/html/const.js",
        "scope": "共通画面操作、フォーム/リンクの二重実行防止、確認、待機表示、トースト、数値表示、操作補助、公開JavaScript API",
        "difference": "Ver2のプルダウン遷移とIE/Netscape向け右クリック禁止を、現行は画面操作支援・二重送信防止・確認モーダル・トースト・表示設定へ置換",
        "intent": "現行仕様を維持",
        "note": "現行の待機カウント、施設説明、選択行、戻る、確認フォーム、トースト位置、数値区切りは各テンプレートのdata属性から参照される。postNavigate/safeFetchは公開APIとして残すが、現時点のテンプレートからは未使用である。Ver2のjumpは現行画面に利用箇所がなく、右クリック禁止は操作性を損ねるだけで防御にもならないため復元しない。",
    },
    {
        "file": "html/new_common.css",
        "v2_source": "旧版_ver2/html/cgi.css",
        "scope": "全画面の色・文字・リンク、共通余白、フッター、エラー/結果表示、トースト・処理中オーバーレイ・確認モーダルの基盤",
        "difference": "Ver2のtable/TD/THとtd1/td2/b1/b2中心の固定CSSを、現行はテーマ変数・共通UI部品・フレックス配置・レスポンシブ前提の基盤CSSへ置換",
        "intent": "現行仕様を維持",
        "note": "背景色、本文色、リンク色、ボタンの青系配色はVer2の印象を引き継ぐ。Ver2のtd1/td2/b1/b2は現行テンプレートに参照がなく、旧table直書き画面とともに復元しない。現行はbase.htmlのトースト、操作ロック用オーバーレイ、確認モーダル、共通フッターを支え、new_game.cssとmobile.cssが画面別/狭幅補正を担当する。",
    },
    {
        "file": "html/new_game.css",
        "v2_source": "旧版_ver2/html/cgi.css / html/choco.css",
        "scope": "ゲーム各画面のレイアウト、カード、フォーム、表、ランキング、戦闘、チョコボ牧場/レース、結果画面、状態別装飾",
        "difference": "Ver2の共通table/TDとチョコボ専用table背景CSSを、現行は画面別の意味的クラス・グリッド/フレックス・カード・可変幅テーブル・レース中継へ置換",
        "intent": "現行仕様を維持",
        "note": "全テンプレートの静的クラス498件のうち420件はnew_common.css/new_game.css/mobile.cssで直接定義し、残りは親スタイル継承・修飾子・動的状態用であることを確認した。旧CSSのtd1/td2/b1/b2と固定幅のチョコボ表はJinjaテンプレートに参照がなく、現行の画面別カード/表/狭幅対応を維持する。通常・王者戦のコース再生はVer2のレース実況意図を現在のブラウザ向けに再実装したものとして含む。",
    },
    {
        "file": "html/mobile.css",
        "v2_source": "該当なし（旧版_ver2はviewport/メディアクエリを持たない）",
        "scope": "768px以下のレイアウト一列化、タップ領域、フォーム操作、表/戦闘/チョコボレースの狭幅表示",
        "difference": "現行専用のモバイル補正を追加。Ver2は固定幅tableと旧ブラウザ向け画面だけで、スマートフォン向け分岐がない",
        "intent": "現行仕様を維持",
        "note": "base.htmlのviewportとmedia属性で狭幅だけに適用する。主要レイアウト、操作列、戦闘HP、牧場、ランキング、レース中継を1列または幅制限へ変更し、送信/ボタンは44px以上を確保する。mobile.cssの65クラスセレクタは全てnew_common.cssまたはnew_game.cssに親定義があり、独立した孤立スタイルはない。",
    },
    {
        "file": "templates/.htaccess",
        "v2_source": "旧版_ver2/.htaccess / 設置方法.txtの公開制限例",
        "scope": "JinjaテンプレートのHTTP直アクセス、Apache 2.2/2.4互換の認可、CGIからのファイル読込",
        "difference": "Ver2はテンプレートディレクトリを持たず、設置例では.pl/.ini/.datを公開禁止にする。現行はテンプレート専用ディレクトリをHTTPから全面非公開にする",
        "intent": "現行仕様を維持",
        "note": "mod_authz_coreではRequire all denied、旧互換ではOrder allow,denyとDeny from allを適用する。これはHTTP要求だけを拒否し、sub_def/utils.pyのFileSystemLoaderによるCGIプロセス内読込を妨げない。テンプレートの生ソースとJinja式を公開しないための現行の分離保護として維持する。",
    },
    {
        "file": "index.html",
    ...
    },
    {
        ...
        "v2_source": "旧版_ver2/html/index.html",
        "scope": "htmlディレクトリの既定応答、ゲーム画面からの参照、ディレクトリ一覧表示の抑止",
        "difference": "内容はVer2と同じ空HTMLであり、現行側の追加仕様はない",
        "intent": "現行仕様を維持",
        "note": "ゲーム本体・テンプレートからhtml/index.htmlへの参照はない。htmlディレクトリはCSSとマニュアルを公開するため、空の既定ページを残してWebサーバーのディレクトリ一覧表示を避ける。削除するとサーバー設定によっては一覧表示または既定応答が変わるため、Ver2同様に維持する。",
    },
    {
        "file": "html/manual.html",
        "v2_source": "旧版_ver2/html/ffhelp.html / html/syokugyou.html",
        "scope": "初心者向けの遊び方、公開職業・必殺技、能力値、施設、冒険エリア、公開範囲、現行値との整合",
        "difference": "Ver2のShift_JIS固定tableヘルプを、現行はUTF-8・目次・カード/表・モバイル対応の単一マニュアルへ再構成。現行固有の施設・戦闘・職業仕様を追記",
        "intent": "公開範囲は現行仕様を維持／宿屋料金表記は不具合を修正済み",
        "note": "職業マスター31種・戦術マスター78技に対し、マニュアルは公開職23種・公開技67種を掲載し、隠し職業/技を伏せる現行の案内方針を維持する。職業名と掲載技名をマスターと照合済み。必殺率の基本式、ジョブLv60でのマスター、敗北時の通常/伝説戦の所持金1%化、待機の説明は現行実装と一致する。宿屋料金だけが旧いLv×1G表記だったため、config.pyのinn_cost_per_level=10に合わせてLv×10Gへ修正した。",
    },
    {
        "file": "html/manual.css",
        "v2_source": "旧版_ver2/html/ffhelp.html / html/syokugyou.html の固定table表示と旧版共通スタイル",
        "scope": "プレイマニュアルのデスクトップ配色・文字組み・目次・職業カード・必殺技表・定義カード・注意書き・ページ先頭への操作",
        "difference": "Ver2の固定幅table中心のヘルプ表示に対し、現行はCSS変数とコンポーネント化したカード/グリッド、横スクロール可能な表、固定目次、レスポンシブのデスクトップ基準スタイルを持つ",
        "intent": "現行仕様を維持",
        "note": "manual.htmlがmanual.cssとmanual-mobile.cssを組み合わせて読み込む構成を確認した。デスクトップ側の各表示要素は対応するセレクタを持ち、表の最小幅とラッパーの横スクロールにより小画面での破綻を避ける。静的なclass照合で未使用に見える一部のユーティリティ/将来用セレクタは、表示・互換性に害がなく他の規則的なスタイル定義と一体のため削除しない。",
    },
    {
        "file": "html/manual-mobile.css",
        "v2_source": "該当なし（旧版_ver2/html/ffhelp.html / html/syokugyou.html はモバイル専用スタイルを持たない）",
        "scope": "幅768px以下でのプレイマニュアルの文字・余白・見出し補足・職業カード・定義カードの表示",
        "difference": "Ver2にない現行追加。デスクトップ基準のmanual.cssを上書きし、狭い画面で余白を縮小、補足を省略、複数列カードを一列に切り替える",
        "intent": "現行仕様を維持",
        "note": "manual.htmlのviewport指定とmanual-mobile.css読込を確認した。上書き対象のcontent-lg、manual-hero、manual-section__sub、manual-job-grid、manual-def-gridはいずれも本体に存在し、768px以下だけに限定される。情報を削る操作は見出しの補足文のみで、職業・必殺技・能力値などの本文は維持される。",
    },
    {
        "file": "backups/.htaccess",
        "v2_source": "旧版_ver2/.htaccess（ルート全面拒否）/ 設置方法.txt内の拡張子別公開制限例",
        "scope": "日次バックアップ世代およびマニフェストのHTTP直接取得防止、Apache 2.2/2.4認可互換",
        "difference": "Ver2ツリーのルート.htaccessも全面拒否だが、日次バックアップ専用ディレクトリはない。現行はバックアップ専用ディレクトリをRequire all deniedまたはDeny from allで全面拒否する",
        "intent": "現行仕様を維持",
        "note": "backups配下には複数の日次バックアップ世代があり、実データは.gitignoreで追跡外、.htaccessだけを追跡する。ディレクトリ単位の全面拒否は、形式が増えてもバックアップ内容を誤公開しないための現行運用上の保護であり、Ver2の拡張子限定より適切である。CGIのファイル操作にはHTTP認可が影響しない。",
    },
    {
        "file": "lock/.htaccess",
        "v2_source": "旧版_ver2/.htaccess（ルート全面拒否）/ 設置方法.txt内の拡張子別公開制限例",
        "scope": "ユーザー・共有データ・バックアップ処理の排他ロック状態のHTTP直接取得防止、Apache 2.2/2.4認可互換",
        "difference": "Ver2ツリーのルート.htaccessも全面拒否だが、専用ロックディレクトリはない。現行はlockディレクトリをRequire all deniedまたはDeny from allで全面拒否する",
        "intent": "現行仕様を維持",
        "note": "現行はlock/<名前>.lockの空ディレクトリをos.mkdirで原子的に作成し、ユーザーID、王者、掲示板、バックアップなどの排他に使う。HTTPからロック名・存在を取得できると処理状態を漏らすため、ディレクトリ全体の拒否を維持する。CGIはConfig['lock_dir']をローカルファイルシステムとして利用するため、この認可設定の影響を受けない。残存ロックは設定値に従い空ディレクトリだけ自動回復する。",
    },
    {
        "file": "save_data/.htaccess",
        "v2_source": "旧版_ver2/.htaccess（ルート全面拒否）/ 設置方法.txt内の拡張子別公開制限例",
        "scope": "個別ユーザー保存データ、パスワードハッシュ、共有状態、チョコボ・掲示板・王者・ランキングキャッシュのHTTP直接取得防止、Apache 2.2/2.4認可互換",
        "difference": "Ver2ツリーのルート.htaccessは全面拒否で、保存専用ディレクトリを分けない。現行はsave_data専用ディレクトリだけを同じ認可構文で全面拒否する",
        "intent": "現行仕様を維持",
        "note": "save_dataにはユーザー別user_all.json、保護ユーザー退避、共有JSON、ログ/キャッシュが置かれ、ユーザー情報やパスワードハッシュを含み得る。Config['save_dir']を通じたCGIのローカル読書き、日次バックアップ、管理復元はHTTP認可の対象外である。形式や新しい共有ファイルが増えても漏れないディレクトリ単位の拒否を維持する。",
    },
    {
        "file": "tools/.htaccess",
        "v2_source": "旧版_ver2/.htaccess（ルート全面拒否）。旧版Ver2には現行tools相当の資料生成ディレクトリはない",
        "scope": "管理者向け文書・装備カタログ・Ver2比較台帳を生成する開発スクリプトのHTTP直接実行/取得防止、Apache 2.2/2.4認可互換",
        "difference": "Ver2には専用の開発ツールディレクトリがない。現行はtoolsディレクトリをRequire all deniedまたはDeny from allで全面拒否する",
        "intent": "現行仕様を維持",
        "note": "tools配下のPythonは運用者がローカルCLIで実行する資料生成用であり、ゲームのCGI入口・テンプレート・公開HTMLからHTTP参照されない。HTTP実行を許す必要はなく、生成ロジックや管理文書の材料を露出しないため全面拒否を維持する。ローカルでのPython実行とGit管理には影響しない。",
    },
    {
        "file": "tools/generate_admin_docs.py",
        "v2_source": "該当なし（旧版Ver2に現行の管理資料HTML生成ツールはない）",
        "scope": "職業・モンスター・保存キー・戦闘・必殺技・チョコボ・移行・運用・セキュリティ資料の生成、正本データ参照、資料間リンク",
        "difference": "Ver2はソースと静的説明だけで、現行はconfig.py・JSONマスター・skills.pyを読み、管理資料をUTF-8 HTMLとして再生成する",
        "intent": "現行仕様を維持／資料の不整合を修正済み",
        "note": "全10出力を監査用に生成し、JSON読込・設定キー参照・Python構文を確認した。必殺技、モンスター技、装飾品効果の対応表に未実装・呼出し不一致はない。資料一覧のリンク先が存在しないdocs/index.htmlだったため、正規入口document_index.htmlへ統一し、設置・運用手順も共通ナビへ追加した。チャンプ時間切れ時の王者交代なし、および上級職でのカルマ固定加算を現行実装に合わせて戦闘仕様書へ反映した。",
    },
    {
        "file": "tools/generate_equipment_catalog.py",
        "v2_source": "該当なし（旧版Ver2に現行の装備カタログHTML生成ツールはない）",
        "scope": "武器・防具・アクセサリーマスターの職業別集約、特殊/初期装備の分離、HTMLエスケープ、カタログ出力先",
        "difference": "Ver2は装備データを個別のテキスト形式で持ち、横断して確認する生成カタログはない。現行は3つのJSONマスターからUTF-8 HTMLを生成する",
        "intent": "現行仕様を維持",
        "note": "武器162件・防具131件・アクセサリー87件、計380マスターを検証した。複数職で使える装備を各職へ掲載するため、出力配置は786件になる。未定義の職業IDはなく、job_ids空配列の初期/特殊装備3件だけを別節へ分離する。再生成結果は既存docs/equipment_catalog.htmlと完全一致し、全職業節とアンカーも一意である。",
    },
    {
        "file": "tools/generate_ver2_comparison_checklists.py",
        "v2_source": "該当なし（旧版Ver2に現行の比較台帳生成ツールはない）",
        "scope": "Ver2マスター/CGIと現行JSON/Pythonの比較表生成、ルート・操作漏れ検出、ファイル単位精査記録、Markdown出力のリンク整合",
        "difference": "Ver2には横断的な比較台帳や静的な漏れ検出がなく、個別データ・CGIを直接確認する運用だった。現行は比較根拠と判断を11個のMarkdown資料へ生成する",
        "intent": "現行仕様を維持",
        "note": "再生成時にFUNCTION_MAPの全38ルートと122操作を確認し、現行ソース/テンプレート内の静的mode値に未記載はない。9モンスターファイル347件、10チョコボファイル495件、モンスター技22種・使用220件を照合した。必須根拠スニペット、比較資料11ファイル、資料内Markdownリンクも検証済み。差異を自動でVer2へ寄せる処理は持たず、FILE_AUDITSへ現行維持・不具合・要判断の根拠を記録する。",
    },
)
...
def _require_command_source_snippets() -> None:
    ...
    required = {
        "login.py": ("FUNCTION_MAP", "POST_ONLY_ROUTE_MODES", '"shop"', "token_check(FORM, session)"),
        "others.py": ("def main",), "chara_make.py": ("mode == \"make_pre\"", "mode == \"make_end\""),
        "cgi_py/shop.py": ("def main", "common.require_owner(user_id)"),
        ...
        "cgi_py/morifarm.py": ("choco_buyb", "choco_sell", "yadoya"), "cgi_py/crace.py": ("race_dendo", "rival_count = 5", "race7", "race8"),
...
def command_action_comparisons(actions: list[dict[str, str]]) -> dict[str, dict[str, str]]:
    ...
    specific = {
        ...
        "レジェンド挑戦を中断して街へ戻る": ("POST+CSRFで進行値をリセット", "legend_cancelをPOST+CSRFで検証する。通常の街画面へ戻った場合も、途中のboss_flagは設定初期値へ戻す。"),
        ...
        "転職画面を表示": ("職業マスターをJSONから参照", "未経験職は能力・職歴前提を満たす候補、経験済み職は保存済み熟練度がLv1以上なら前提を免除した再転職候補として表示する。未マスター候補は別表示する。"),
        "転職を実行": ("未経験職だけ前提を再検証", "現職Lvを退避、転職先Lvを復帰し、必要なら戦術を初期化する。経験済み職は能力・カルマ・必要マスター職を免除するが、未経験職は実行POSTでも再検証する。成功時はVer2と同じく接続元ホストを更新し、chara/syokuを同時保存する。能力減少値は一致する。カルマ下限はVer2が0を許容し現行は1へ戻すが、再精査のうえ最低1とする現行仕様をユーザーが明示決定した。", "現行仕様を維持（明示決定）"),
        ...
        "装備中の武器を倉庫へ外す": ("着脱時の接続元保存を現行は省略", "武器倉庫8件を確認してマスター値を退避し、素手へ戻す。Ver2はREMOTE_ADDRを保存するが、ユーザー明示決定により現行は装備・倉庫だけを更新する。", "現行仕様を維持（明示決定）"),
        "倉庫の武器を装備": ("現行は選択品を除去し、旧装備を末尾へ追加", "Ver2は選択した保管位置を旧装備で置換する。ユーザー明示決定により現行の末尾追加方式を維持する。職業制限はVer2・現行とも倉庫交換時に再検証しない。", "現行仕様を維持（明示決定）"),
        "倉庫の武器を削除": ("Ver2の二段階確認を共通モーダルへ置換", "Ver2は確認画面を挟んでから削除する。現行はCSRF付きPOSTの直前に、破棄対象名を示す共通確認モーダルを表示する。", "不具合を修正（明示決定）"),
        "装備中の防具を倉庫へ外す": ("着脱時の接続元保存を現行は省略", "防具倉庫8件を確認してマスター値を退避し、衣服へ戻す。Ver2はREMOTE_ADDRを保存するが、ユーザー明示決定により現行は装備・倉庫だけを更新する。", "現行仕様を維持（明示決定）"),
        "倉庫の防具を装備": ("現行は選択品を除去し、旧装備を末尾へ追加", "Ver2は選択した保管位置を旧装備で置換する。ユーザー明示決定により現行の末尾追加方式を維持する。職業制限はVer2・現行とも倉庫交換時に再検証しない。", "現行仕様を維持（明示決定）"),
        "倉庫の防具を削除": ("Ver2の二段階確認を共通モーダルへ置換", "Ver2は確認画面を挟んでから削除する。現行はCSRF付きPOSTの直前に、破棄対象名を示す共通確認モーダルを表示する。", "不具合を修正（明示決定）"),
        "装備中の装飾品を倉庫へ外す": ("着脱時の接続元保存を現行は省略", "装飾品倉庫8件を確認してマスター値を退避し、補正なしへ戻す。Ver2はREMOTE_ADDRを保存するが、ユーザー明示決定により現行は装備・倉庫だけを更新する。", "現行仕様を維持（明示決定）"),
        "倉庫の装飾品を装備": ("現行は選択品を除去し、旧装備を末尾へ追加", "Ver2は選択した保管位置を旧装備で置換する。ユーザー明示決定により現行の末尾追加方式を維持する。", "現行仕様を維持（明示決定）"),
        "倉庫の装飾品を削除": ("Ver2の二段階確認を共通モーダルへ置換", "Ver2は確認画面を挟んでから削除する。現行はCSRF付きPOSTの直前に、破棄対象名を示す共通確認モーダルを表示する。", "不具合を修正（明示決定）"),
        ...
        "防具を売却": ("装備・所持金を統合JSONで同時保存し、結果をトースト表示", "装備中だけをマスター価格の2/3で下取りし、gold上限で打ち止めにして衣服へ戻す。Ver2は売却時にもREMOTE_ADDRを保存するが、現行は保存しない。再精査で保存省略の履歴根拠がないことを確認したうえで、売却時も接続元ホストを更新しない現行仕様をユーザーが明示決定した。", "現行仕様を維持（明示決定）"),
        ...
        "装飾品を売却": ("装備・所持金を統合JSONで同時保存し、結果をトースト表示", "装備中だけをマスター価格の2/3で下取りし、gold上限で打ち止めにして補正なしへ戻す。Ver2は売却時にもREMOTE_ADDRを保存するが、現行は保存しない。再精査で保存省略の履歴根拠がないことを確認したうえで、売却時も接続元ホストを更新しない現行仕様をユーザーが明示決定した。", "現行仕様を維持（明示決定）"),
        ...
        "通常モンスター修行": ("POST+CSRF・統合保存へ移行", "battle_count>0と出現表・修行回数を検査し、勝敗別報酬・戦績・職歴を保存する。戦闘後HPはレベルアップ後のvit/max_hpで回復する。敗北EXPはVer2と同じ1へ復元した。", "不具合を修正（明示決定）"),
        "幻影の城へ挑戦": ("monster.pyのgenei分岐へ統合", "battle_count>0、出現条件、修行回数、敵攻撃への防具補正、幻影報酬を通常修行と区別して処理する。戦闘後HPはレベルアップ後のvit/max_hpで回復する。敗北EXPはVer2と同じ1へ復元した。", "不具合を修正（明示決定）"),
        "異世界へ挑戦": ("monster.pyのisekiai分岐へ統合", "battle_count>0、レベル/進行条件と修行回数を検査し、異世界出現表と報酬を用いる。戦闘後HPはレベルアップ後のvit/max_hpで回復する。敗北EXPはVer2と同じ1へ復元した。", "不具合を修正（明示決定）"),
        "レジェンド攻略者一覧を閲覧": ("公開GET閲覧を明示", "ログイン不要で攻略者を称号・戦績から並べ、状態変更は行わない。"),
        "レジェンドの階層へ挑戦": ("POST専用・階層値を検証、クリア後の再挑戦を許可", "battle_count>0、title_id、boss_flag、修行回数、待機時間を検査して階層別モンスターと戦う。クリア後は進行値を開始値へ戻し、現行は同じ階層を再挑戦できる。攻略者向けの全体メッセージと新称号の表示は、title_idが実際に上がる初回攻略時だけ行う。戦闘後HPはレベルアップ後のvit/max_hpで回復する。"),
        "天下一武道会ロビーを表示": ("24時間メンバーキャッシュへ移行", "battle_count>0を検査し、レベル上位者、参加可能boss_flag、進行ラウンド、制覇履歴を表示する。"),
        "天下一武道会で対戦": ("ラウンド番号をサーバー側照合、戦闘全体をユーザーロックで直列化", "battle_count>0と期待ラウンドを検査する。Ver2は結果を問わず進行するが、現行は勝利だけ進行する。03a81a1で時間切れを進めないことを確認し、相打ち・敗北も進める合理的根拠がないため、再精査のうえ勝利のみ進行する現行仕様をユーザーが明示決定した。戦闘前から保存までロックし、レベルアップ後のvit/max_hpで戦闘後HPを回復、boss_flagが開始値を超える場合は開始値へ正規化する。", "現行仕様を維持（明示決定）"),
    ...
    for action in actions:
        ...
        if name in specific:
            ...
            if name.startswith("チョコボ訓練:"):
                ...
                difference, note, intent = "殿堂JSONをライバル表へ利用し、5頭をランダム抽選", "Ver2は殿堂一覧から5頭を指定する。現行は入力を受けず、自チョコボに加えてdenchoco.jsonから5頭をランダム抽選するようユーザーが明示決定した。", "現行仕様調整（明示決定）"
            elif name == "チョコボ殿堂を表示":
                difference, note, intent = "JSON一覧へ移行し、閲覧をログイン済み利用者に限定", "Ver2は未ログインでも公開するが、現行のログイン必須をユーザーが明示決定して維持する。", "現行仕様を維持（明示決定）"
            elif name == "チョコボを殿堂登録":
                difference, note, intent = "JSON・CSRF・本人認証へ移行し、同一登録は元位置で更新", "Ver2は同一ID・同名の更新も先頭へ移動するが、現行の元位置更新をユーザーが明示決定して維持する。", "現行仕様を維持（明示決定）"
            elif name == "チョコボを手放す":
    ...
    write_static_checklist(
        ...
        (
            ...
            ("ダメージ確定・HP・勝敗", (
                ...
                ("対人長期戦の瀕死時10倍補正", "旧版_ver2/wbattle.pl:battle_kaihi", "sub_def/battle_logic.py:BattleSimulator.simulate", "両者がHP10%未満かつ16ターン目以降の攻撃10倍補正、回避判定との優先順"),
...
def battle_logic_comparisons() -> dict[str, dict[str, str]]:
    ...
    items = {
        ...
        "選択戦術IDの取得": ("列番号からtactic_idへ名称化", "意図的", "確認済み", "プレイヤー・対人相手とも選択戦術IDを使用する。欠損・変換不能・負値は0へ正規化し、存在しない正のIDは未定義メソッドとして無処理になる。現職ではないマスター戦術も選択済みなら実行する仕様を維持する。"),
        ...
        "対人長期戦の瀕死時10倍補正": ("Ver2の瀕死時10倍補正を復元", "不具合修正（明示決定）", "確認済み", "対人戦で相手HPが相手最大HPの10%未満、かつ自分の現在HPの10%未満、かつ16ターン目以降の場合に相手攻撃を10倍にする。双方に対称の判定があり、成立時は通常の回避判定を行わない。Ver2のwbattle.pl:battle_kaihiと同じく、防御・上級職軽減の後に適用する。"),
        "先行攻撃による敵行動停止": ("部分的な敵行動停止を撤廃して同時精算へ復帰", "不具合修正（明示決定）", "確認済み", "Ver2と同様、双方の行動後にHPを同時精算する。敵がこのターンに倒れても、同ターンに成立した敵ダメージは受ける。速度・先攻後攻は導入しない。"),
        "ドレイン回復の基準": ("Ver2の設定ダメージ基準から実ダメージ基準へ変更", "意図的", "確認済み", "防御・回避後のdmg×割合で回復する現行仕様を維持する。ドレイン43の全量回復など個別割合はskills.mdで調整済み。"),
        "HP・回復・自傷の精算順": ("回復を最大HPまでに制限してから同時精算", "現行仕様を維持（明示決定）", "確認済み", "双方が同時に行動し、各回復は現在HPから最大HPまでで打ち止めてから被ダメージ・自傷を差し引く。過剰回復を致死ダメージの緩衝材にしない。"),
        ...
        "通常・幻影・異世界の報酬処理": ("結果処理をmonster.pyへ分離。回復順・修行回数・敗北EXPをVer2準拠へ修正", "不具合修正（明示決定）", "確認済み", "勝敗別EXP/G、盗み差分、修行回数、幻影宝箱を入口側で処理する。通常・幻影・異世界は残り回数0で開始できず、各戦闘で1回消費する。戦闘後HPはlevelup後のvit/max_hpで回復する。初期Python移植から0だった敗北EXPは、ユーザー明示決定によりVer2と同じ1へ復元した。時間切れを引分として扱う現行差は上記結果コード行と連動する。"),
        "レジェンドの報酬・階層進行": ("結果処理をlegend.pyへ分離。クリア後の再挑戦を追加し、称号が実際に上がる時だけ攻略通知", "現行仕様を維持（明示決定）", "確認済み", "勝利・引分・敗北のEXP/G・battle_count・win_count・battle_limit・進行初期化は対応する。戦闘後HPはVer2のhp_afterと同じくlevelup後のvit/max_hpで回復する。Ver2は最終ボス撃破時にboss_flag=0を保持して連戦を終えるが、現行は開始値10へ戻して同階層を再挑戦可能にする。全体メッセージと新称号の表示は、title_idが実際に上がる初回攻略時だけ行う。"),
...
def ownership_progression_comparisons() -> dict[str, dict[str, str]]:
    ...
    results = {
        ...
        "修行回数・待機時刻": {
            ...
            "intent": "意図的／待機20秒は現行仕様を維持（明示決定）",
            "status": "確認済み",
            "note": "通常・幻影・異世界・レジェンドは修行回数を消費し、チャンピオン・天下一は結果にかかわらず上限まで補充する。各戦闘CGIは完了時にlast_timeを更新し、設定値の待機時間で検査する。待機時間はVer2の30秒に対して現行20秒であり、二次精査でユーザーが維持を明示決定した。",
        },
        "レジェンドの進行フラグ・称号": {
            "difference": "進行列・称号列をboss_flag/title_idへ名称化し、クリア後は開始値へ戻して再挑戦可能化",
            "intent": "現行仕様を維持（明示決定）",
            "status": "確認済み",
            "note": "階層選択は称号値で制限し、勝利でboss_flagを減算、敗北・引分では開始値へ戻す。Ver2は階層クリア時にboss_flag=0を保持し、結果画面から続行できない。現行はboss_flagを開始値へ戻して再挑戦可能にする。title_idが実際に上がる初回攻略時だけ、全体メッセージの攻略通知と「新しい称号」の表示を行う。実際に加算するEXPも結果へ表示する。",
        },
        "人間チャンピオン": {
        ...
        "引退・お見合い候補リスト": {
            ...
            "intent": "現行仕様を維持（明示決定）",
            "status": "確認済み",
            "note": "性別別リストへ引退個体の戦績・血統・価格を移し、配合相手の候補にする基本設計は同じ。Ver3は実リスト長を基準に置換し、空リストも許容してchocobo_partner_list_limit（性別ごと100件）で切り詰める。固定99枠は採用しないが、Ver2どおり同じ育て親の引退個体を25%で通常候補とは別に追加表示する。",
        },
        "配合後の子チョコボ": {
        ...
        "チョコボ殿堂の共有リスト": {
            "difference": "denchoco行レコードからJSON＋トロフィー名の埋込へ移行。現行は一覧をログイン済み利用者に限定し、同一ID・同名を元位置で上書きする",
            "intent": "現行仕様を維持（明示決定）",
            "status": "確認済み",
            "note": "Ver2は同一ID・同名を更新時にも先頭へ移動し、異なる個体も先頭追加する。現行は同一登録を元位置で上書きする。登録時はVer2と同じく重賞数を問わず、名前付きの現役チョコボを対象とする。Ver2では公開一覧を未ログインでも閲覧できたが、現行はa533b17の状態変更向けIDOR対策でrequire_ownerを無条件に呼ぶためログイン済み利用者に限られる。二次精査でユーザーが登録順と一覧公開範囲の現行維持を明示決定した。",
        },
        "チョコボ王者": {
...
def storage_migration_comparisons() -> dict[str, dict[str, str]]:
    ...
    return {
        ...
        "ログイン・ログアウト": {
            ...
            "intent": "意図的",
            "status": "差異あり",
            "note": "平文パスワードを含むVer2のログイン履歴機能は廃止で確定する。移行済みの過去ログは互換データとして残すが、login.pyから新規の成功・失敗履歴は追加しない。",
        },
        "パスワード形式とログイン時移行": {
