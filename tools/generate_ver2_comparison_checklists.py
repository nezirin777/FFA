...
FILE_AUDITS = (
    ...
    },
    {
        ...
        "note": "3種別の上限は8で一致。現行は選択品を配列から取り出し、旧装備を末尾へ追加する方式、および着脱時にREMOTE_ADDRを保存しない方式を維持する。破棄はCSRF付きPOSTの直前に共通確認モーダルを表示する。",
    },
    {
    ...
    },
    {
        ...
        "intent": "処理移行・防具2181・売却時接続元は現行維持（明示決定）",
        "note": "購入・売却額は両版とも価格の2/3、購入時のREMOTE_ADDR保存と防具倉庫上限8は一致する。Ver2は売却時にもREMOTE_ADDRを保存するが、現行は保存しない仕様を維持する。2165・2183の通常長音符への統一と、皇帝用2181の販売追加は再照合後に現行維持を明示決定した。",
    },
    {
        ...
        "intent": "処理移行・売却時接続元は現行維持（明示決定）",
        "note": "購入・売却額は両版とも価格の2/3、購入時のREMOTE_ADDR保存と装飾品倉庫上限8は一致する。Ver2は売却時にもREMOTE_ADDRを保存するが、現行は保存しない仕様を維持する。職別品揃えと補正は一致し、85・87の率補正はd7105f6のVer1準拠調整として、再照合後に現行維持を明示決定した。",
    },
    {
    ...
    },
    {
        ...
        "note": "weapon/armor/accessoryのremove・equip・delete全9モードは、テンプレート送信のsを状態変更前に検証するよう修正済み。装備交換時は現在装備を倉庫へ戻し、容量上限は取り外し時に確認する。破棄は対象の保管枠だけを削除し、送信前に共通確認モーダルを表示する。",
    },
    {
...
def command_action_comparisons(actions: list[dict[str, str]]) -> dict[str, dict[str, str]]:
    ...
    specific = {
        ...
        "装備中の武器を倉庫へ外す": ("着脱時の接続元保存を現行は省略", "武器倉庫8件を確認してマスター値を退避し、素手へ戻す。Ver2はREMOTE_ADDRを保存するが、現行は装備・倉庫だけを更新する。現行仕様を維持する。", "現行仕様を維持（明示決定）"),
        "倉庫の武器を装備": ("現行は選択品を除去し、旧装備を末尾へ追加", "Ver2は選択した保管位置を旧装備で置換する。現行の並び替えを伴う方式を維持する。職業制限はVer2・現行とも倉庫交換時に再検証しない。", "現行仕様を維持（明示決定）"),
        "倉庫の武器を削除": ("Ver2の二段階確認を共通モーダルへ置換", "Ver2は確認画面を挟んでから削除する。現行はCSRF付きPOSTの直前に、破棄対象名を示す共通確認モーダルを表示する。", "不具合を修正（明示決定）"),
        "装備中の防具を倉庫へ外す": ("着脱時の接続元保存を現行は省略", "防具倉庫8件を確認してマスター値を退避し、衣服へ戻す。Ver2はREMOTE_ADDRを保存するが、現行は装備・倉庫だけを更新する。現行仕様を維持する。", "現行仕様を維持（明示決定）"),
        "倉庫の防具を装備": ("現行は選択品を除去し、旧装備を末尾へ追加", "Ver2は選択した保管位置を旧装備で置換する。現行の並び替えを伴う方式を維持する。職業制限はVer2・現行とも倉庫交換時に再検証しない。", "現行仕様を維持（明示決定）"),
        "倉庫の防具を削除": ("Ver2の二段階確認を共通モーダルへ置換", "Ver2は確認画面を挟んでから削除する。現行はCSRF付きPOSTの直前に、破棄対象名を示す共通確認モーダルを表示する。", "不具合を修正（明示決定）"),
        "装備中の装飾品を倉庫へ外す": ("着脱時の接続元保存を現行は省略", "装飾品倉庫8件を確認してマスター値を退避し、補正なしへ戻す。Ver2はREMOTE_ADDRを保存するが、現行は装備・倉庫だけを更新する。現行仕様を維持する。", "現行仕様を維持（明示決定）"),
        "倉庫の装飾品を装備": ("現行は選択品を除去し、旧装備を末尾へ追加", "Ver2は選択した保管位置を旧装備で置換する。現行の並び替えを伴う方式を維持する。", "現行仕様を維持（明示決定）"),
        ...
        "防具を売却": ("装備・所持金を統合JSONで同時保存し、結果をトースト表示", "装備中だけをマスター価格の2/3で下取りし、gold上限で打ち止めにして衣服へ戻す。Ver2は売却時にもREMOTE_ADDRを保存するが、現行は保存しない仕様を維持する。", "現行仕様を維持（明示決定）"),
        ...
        "装飾品を売却": ("装備・所持金を統合JSONで同時保存し、結果をトースト表示", "装備中だけをマスター価格の2/3で下取りし、gold上限で打ち止めにして補正なしへ戻す。Ver2は売却時にもREMOTE_ADDRを保存するが、現行は保存しない仕様を維持する。", "現行仕様を維持（明示決定）"),
