...
def command_action_comparisons(actions: list[dict[str, str]]) -> dict[str, dict[str, str]]:
    ...
    specific = {
        ...
        "道場の入口を表示": ("テンプレート入口とPOST+CSRFへ移行", "名前の完全一致検索または一覧選択へ進むだけで、保存状態は変更しない。"),
        "対人相手を選択": ("相手IDをサーバー側で再検証・本人選択を拒否", "存在しない相手と本人を拒否して選択画面だけを表示する。Ver2の候補一覧は24時間キャッシュ済みレベル降順だが、現行は全ユーザーを直接列挙するため表示順を保証しない。"),
        "選択相手と対戦": ("模擬戦を保存しないシミュレーションへ明示", "相手の現在のキャラクター・装備で戦い、経験値・所持金・戦績・待機時刻を更新しない。時間切れwin=3は相打ちと別表示にする。"),
        "通常モンスター修行": ("POST+CSRF・統合保存へ移行", "出現表と修行回数を検査し、勝敗別報酬・戦績・職歴を保存する。戦闘後HPはレベルアップ後のvit/max_hpで回復する。"),
        "幻影の城へ挑戦": ("monster.pyのgenei分岐へ統合", "出現条件、修行回数、敵攻撃への防具補正、幻影報酬を通常修行と区別して処理する。戦闘後HPはレベルアップ後のvit/max_hpで回復する。"),
        "異世界へ挑戦": ("monster.pyのisekiai分岐へ統合", "レベル/進行条件と修行回数を検査し、異世界出現表と報酬を用いる。戦闘後HPはレベルアップ後のvit/max_hpで回復する。"),
        "レジェンド攻略者一覧を閲覧": ("公開GET閲覧を明示", "ログイン不要で攻略者を称号・戦績から並べ、状態変更は行わない。"),
        "レジェンドの階層へ挑戦": ("POST専用・階層値を検証、クリア後の再挑戦を許可", "title_id、boss_flag、修行回数、待機時間を検査して階層別モンスターと戦う。クリア後は進行値を開始値へ戻し、現行は同じ階層を再挑戦できる。戦闘後HPはレベルアップ後のvit/max_hpで回復する。"),
        "天下一武道会ロビーを表示": ("24時間メンバーキャッシュへ移行", "レベル上位者、参加可能boss_flag、進行ラウンド、制覇履歴を表示する。"),
        "天下一武道会で対戦": ("ラウンド番号をサーバー側照合、戦闘全体をユーザーロックで直列化", "期待ラウンド以外を拒否する。勝敗・進行のwin分岐は現行方針を維持し、戦闘前から保存までロックする。レベルアップ後のvit/max_hpで戦闘後HPを回復し、boss_flagが開始値を超える場合は開始値へ正規化する。"),
...
def battle_logic_comparisons() -> dict[str, dict[str, str]]:
    ...
    items = {
        ...
        "職業別の基礎ダメージ（全31職）": ("9cd9d14のカルマ乱数化を固定加算へ修正", "不具合修正", "確認済み", "Ver2は魅力をrand(cha)、カルマを固定加算する。9cd9d14でカルマ参照をrand(karma)へ変更していたが、固定加算へ戻した。all_stats、職24、職25の3箇所が対象で、該当する上級職の対人計算にも同じ修正が適用される。"),
