...
FILE_AUDITS = (
    ...
    },
    {
        ...
        "difference": "転職成功時の接続元ホスト保存が欠けていた。Ver2はmaster_tacで戦術リセットも制御するが、現行は候補利用と転職時リセットを別設定に分離する。カルマ下限はVer2と異なる",
        "intent": "ホストは不具合を修正済み／戦術リセット設定の分離は現行仕様／カルマ下限1は現行仕様を維持（明示決定）",
        "note": "31d5c1bで成功時にREMOTE_ADDRを保存するよう修正した。能力減少値と職業Lv遷移は一致する。現在の既定値はmaster_tactics_enabled=1・reset_tactics_on_job_change=1でVer2のmaster_tac=1と同じ結果になる。config.pyの説明どおり、現行は他職戦術を候補へ加えるかと転職直後に選択中戦術を解除するかを独立して設定する。Ver2は負値だけを1へ戻すためカルマ0を許容する一方、現行は0も1へ戻す。再精査で履歴根拠がないことを確認したうえで、カルマ最低値を1とする現行仕様をユーザーが明示決定した。",
    },
    {
...
def command_action_comparisons(actions: list[dict[str, str]]) -> dict[str, dict[str, str]]:
    ...
    specific = {
        ...
        "転職を実行": ("実行POSTでも前提職を再検証", "現職Lvを退避、転職先Lvを復帰し、必要なら戦術を初期化する。成功時はVer2と同じく接続元ホストを更新し、chara/syokuを同時保存する。能力減少値は一致する。カルマ下限はVer2が0を許容し現行は1へ戻すが、再精査のうえ最低1とする現行仕様をユーザーが明示決定した。", "現行仕様を維持（明示決定）"),
