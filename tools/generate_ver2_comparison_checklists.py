...
FILE_AUDITS = (
    ...
    },
    {
        ...
        "intent": "ホストは不具合を修正済み／カルマ下限1は現行仕様を維持（明示決定）",
        "note": "成功時にREMOTE_ADDRを保存する。能力減少値と職業Lv遷移は一致する。Ver2は負値だけを1へ戻すためカルマ0を許容する一方、現行は0も1へ戻す。再精査で履歴根拠がないことを確認したうえで、カルマ最低値を1とする現行仕様をユーザーが明示決定した。",
    },
    {
    ...
    },
    {
        ...
        "note": "ログイン本人だけが200文字以内・禁止語なしで投稿でき、最新100件を新着順に共有JSONへ保持する。投稿後のPRGリダイレクト、CSRF、共有ロックは現行の安全化。Ver2の私信・送信箱・受信拒否・友人登録は置き換えず、現行では機能と移行済みデータを廃止する。",
    },
    {
        ...
        "intent": "画面・安全化は意図的／街へ戻る際の進行リセットはVer2準拠（明示決定）",
        "note": "戦闘回数0ではモンスター・幻影・異世界・道場・武道会・レジェンドを表示段階で止め、20秒待機、クラス、宿代Lv×10、称号段階は一致する。Ver2はboss_flagが10以外ならレジェンド入口から挑戦させない。現行も街画面へ入る時点で途中のboss_flagを10へ戻す。戦闘結果画面からの連戦は街を経由しないため、同じ階層内では継続できる。",
    },
    {
    ...
    },
    {
        ...
        "difference": "直接実行時の解放・異世界Lv制限、20秒待機、敗北EXP0、JSON重み抽選へ移行。幻影・異世界でも後続モンスター技フックを呼ぶが、全IDが空処理で実効差はない",
        "intent": "入口検証・待機・敗北EXP0・抽選移行は既決仕様／後続技フックは現行仕様を維持（明示決定）",
        "note": "Ver2の重複行数と現行weight合計は初級193・中級169・上級45・最上級121・異世界95で全て一致する。通常・幻影・異世界の回数消費、勝敗時の所持金、戦績、boss_flag復帰、レベルアップ後のHP回復、接続元保存は確認した。現行はbattle_count>0と異世界Lv300を実行側でも検査し、待機は設定値20秒、敗北EXPは0とする。Ver2は通常修行だけmons_atowazaを呼び、幻影・異世界では呼ばない。現行は全モードで呼ぶが、mons_0〜mons_22の同フックは全て空実装のため実効差はない。現行の共通呼出しを維持する。",
    },
    {
    ...
    },
    {
        ...
        "note": "Ver2の10部門（勝数、訓練数、筋力、スタミナ、粘り、落ち着き、闘争心、賢さ、反射神経、獲得賞金）を各10件・同じ並びで表示する。退役済みの空データはcommon.choco_loadで対象外とし、賞金は両版とも保存値の100倍を表示する。能力画像はVer2の厳密な>100、>200、>400、>600、>800、>1000境界に戻す。キャッシュ版を上げて旧ランク番号を即時再構築する。現行は画像だけだったVer2に実数値も併記し、キャッシュの最終更新時刻も表示する。重賞履歴はr1〜r22を全レース名と○/−で表示する。",
    },
    {
        ...
        "intent": "本人確認・直接POST検証・重賞色分け・自己チョコボ詳細の表示量は不具合を修正済み",
        "note": "クラス境界、体調、一般レースの勝数範囲、G1の40回・G2の60回周期、性別限定最終枠は一致する。牧場の能力画像はVer2もint(能力/100)であり、ランキング画面の厳密な閾値判定とは別仕様のため変更しない。海外重賞の解放はVer2のrenzと同じr1〜r22合計3個以上をcrace.pyでも検証する。現行は待機終了をちょうど設定秒で許可し、Ver2の1秒後判定より操作性を優先する。自己チョコボの画像・性別・血統・訓練数・7能力ランクと実数値を表示へ復元し、ステータスカードを2カラム全幅、訓練・お世話・アドバイス・関連施設を後続の2カラムへ再配置した。",
    },
    {
        ...
        "difference": "JSON・テンプレート・本人認証へ移行。状態変更フォームはCSRFトークンを出力していたが、処理側の検証が欠けていた。配合のprebirth乱数はVer2の合算後切捨てを個別切捨てへ誤移植していた",
    ...
    },
    {
        ...
        "intent": "終盤計算と結果ログの不備を修正済み／20秒待機・所持金上限・外部サイト情報の廃止は現行仕様を維持",
        "note": "体力400以上、同一王者の再挑戦拒否、20ターン、写真判定、勝利時のrun/win・cmax+80・各能力+1〜6・賞金、敗北時のrun・cmax+20・各能力+1・防衛連勝、寿命-200、老衰・能力/総合上限の順序は一致する。終盤はVer2どおり補正前移動量からスタミナ消費を再計算し、両者の闘争心判定をrand(能力×1.5)に戻した。Ver2の待機時間30秒に対し、現行は設定可能な20秒を採用する。結果ログは簡略化を維持しつつ、各ターンの残り距離を表示する。",
    },
    {
        ...
        "intent": "無名個体拒否、負能力処理順、CSRF検証を不具合として修正済み／無効mode拒否と20秒待機は現行仕様を維持",
        "note": "訓練種別と7能力の増減、20回・成功率75%、知力訓練の固定問題、成功のみ/混在/失敗のみの限界突破+3/+5、訓練数・寿命-50・cmax+5、老衰、総合上限時の最大能力限界+2確率1/3は一致する。Ver2の順序どおり、老衰と個別上限を反映してから負能力を1へ戻し最大能力限界を-5する。Ver2は待機30秒で無効mode時にも処理へ進むが、現行は設定可能な20秒と無効mode拒否を採用する。",
    },
    {
    ...
    },
    {
        ...
        "intent": "現行仕様を維持",
        "note": "人間チャンプのクラス（job_level÷10）、勝率、能力バー、命中・回避・必殺率の上限、テスト用アカウントのlast_timeから600秒判定、現役一覧の期限判定は一致する。現行は外部サイトURL・平文パスワードを公開/保存せず、能力値と装備補正・チョコボ王者のクラス/戦績/能力を表示する。",
    },
    {
    ...
    },
    {
        ...
        "intent": "現行仕様を維持",
        "note": "st_buyは画像番号とコメントをサーバー側で検査し、ロック後の最新データへ保存する。テンプレートが送るsをst_buy入口で検証するよう修正済み。トースト位置と数値表記はブラウザlocalStorageだけを更新し、サーバー上のキャラクターデータを変更しない。",
    },
    {
