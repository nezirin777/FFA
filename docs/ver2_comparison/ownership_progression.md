...
| 人間チャンピオン | `旧版_ver2/datalog/winner.cgi / battle.cgi` | `save_data/champion.json / cgi_py/battle.py` | winner.cgiの連番レコードからchampion.jsonの名前付き状態へ移行。敗北EXPに上限追加 | 意図的 | 確認済み | 勝利・引分で挑戦者を王者へ交代し、敗北では王者の連勝・最高連勝を更新する。勝利・引分EXPは相手Lv×基準値、敗北EXPだけmin(相手Lv, 自分Lv×10)へ調整済み。 |
